...
class s9e_MediaBBCodes
{
	...
	*/
	public static $sites = array(
		...
		'espndeportes'=>array('ESPN Deportes','http://www.espndeportes.com/',array('sports'=>1),"!espndeportes\\.com/videohub/video/clipDeportes\\?id=(?'id'\\d+)!\n!(?'id')espndeportes\\.com/\\?id=\\w+&topId=\\d+!",array('!espndeportes\\.com/videohub/video/clipDeportes\\?id=(?\'id\'\\d+)!'),true,array(array('extract'=>array('!espn\\.mem\\.video\\((?\'id\'\\d+)!'),'match'=>array('!espndeportes\\.com/\\?id=\\w+&topId=\\d+!'))),'<iframe width="560" height="315" src="https://espn.go.com/video/iframe/twitter/?cms=deportes&amp;id={$id}" allowfullscreen="" frameborder="0" scrolling="no"></iframe>'),
		...
		'twitch'=>array('Twitch','http://www.twitch.tv/',array('gaming'=>1),"#(?'id')twitch\\.tv/(?'channel'(?!m/)\\w+)(?:/b/(?'archive_id'\\d+)|/c/(?'chapter_id'\\d+))?#\n!(?'id')twitch\\.tv/m/\\d+!",array('#twitch\\.tv/(?\'channel\'(?!m/)\\w+)(?:/b/(?\'archive_id\'\\d+)|/c/(?\'chapter_id\'\\d+))?#'),true,array(array('extract'=>array('!channel=(?\'channel\'\\w+).*?videoId=a(?\'archive_id\'\\d+)!'),'match'=>array('!twitch\\.tv/m/\\d+!')))),
