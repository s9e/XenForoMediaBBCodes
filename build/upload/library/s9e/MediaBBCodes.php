...
class s9e_MediaBBCodes
{
	...
	*/
	public static $sites = array(
		...
		'imgur'=>array('Imgur','http://imgur.com/',array('images'=>1),"@imgur\\.com/(?!r/|user/)(?:gallery/|(?P<type>t)/\\w+/)?(?P<id>(?:a/)?\\w+)(?!\\w|\\.(?:pn|jp)g)@\n@(?P<id>)imgur\\.com/gallery/\\w@",array('@imgur\\.com/(?!r/|user/)(?:gallery/|(?P<type>t)/\\w+/)?(?P<id>(?:a/)?\\w+)(?!\\w|\\.(?:pn|jp)g)@'),true,array(array('extract'=>array('!image\\s*:\\s*.*?"is_(?P<type>album)":true!','!<div id="(?P<type>album)-!','!class="(?P<type>album)-image!'),'match'=>array('@imgur\\.com/gallery/\\w@')))),
	...
	*/
	...
	public static function renderImgur($vars)
	{
		...
		$html='<iframe data-s9e-mediaembed="imgur" allowfullscreen="" onload="var b=Math.random();window.addEventListener(\'message\',function(a){a.data.id==b&amp;&amp;(style.height=a.data.height+\'px\',style.width=a.data.width+\'px\')});contentWindow.postMessage(\'s9e:\'+b,\'https://s9e.github.io\')" scrolling="no" style="border:0;height:450px;max-width:100%;width:568px" src="https://s9e.github.io/iframe/imgur.min.html#';if(!((strpos($vars['id'],'a/')===0)))if($vars['type']==='album'||$vars['type']==='t')$html.='a/';$html.=htmlspecialchars($vars['id'],2).'"></iframe>';
