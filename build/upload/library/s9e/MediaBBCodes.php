...
class s9e_MediaBBCodes
{
	...
	*/
	public static $sites = array(
		...
		'facebook'=>array('Facebook','http://www.facebook.com/',array('social'=>1),"@/(?!(?:apps|developers|graph)\\.)[-\\w.]*facebook\\.com/(?:[/\\w]+/permalink|(?!pages/|groups/).*?)(?:/|fbid=|\\?v=)(?P<id>\\d+)(?=$|[/?&#])@\n#(?P<id>)(?=.*?[./]facebook\\.com[:/]).*?/(?P<type>video)s?/#",array('@/(?!(?:apps|developers|graph)\\.)[-\\w.]*facebook\\.com/(?:[/\\w]+/permalink|(?!pages/|groups/).*?)(?:/|fbid=|\\?v=)(?P<id>\\d+)(?=$|[/?&#])@','#(?=.*?[./]facebook\\.com[:/]).*?/(?P<type>video)s?/#'),true,7=>'<iframe data-s9e-mediaembed="facebook" allowfullscreen="" onload="var a=Math.random();window.addEventListener(\'message\',function(b){if(b.data.id==a)style.height=b.data.height+\'px\'});contentWindow.postMessage(\'s9e:\'+a,\'https://s9e.github.io\')" scrolling="no" src="https://s9e.github.io/iframe/facebook.min.html#{$id}" style="border:0;height:360px;max-width:640px;width:100%"></iframe>'),
