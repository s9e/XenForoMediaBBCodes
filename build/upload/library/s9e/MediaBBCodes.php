...
class s9e_MediaBBCodes
{
	...
	*/
	public static $sites = array(
		...
		'googleplus'=>array('Google+','https://plus.google.com/',array('social'=>1),"!(?'id')//plus\\.google\\.com/(?:u/\\d+/)?(?:\\+\\w+|(?'oid'\\d+))/posts/(?'pid'\\w+)!\n!(?'id')//plus\\.google\\.com/(?:u/\\d+/)?\\+[^/]+/posts/\\w!",array('!//plus\\.google\\.com/(?:u/\\d+/)?(?:\\+\\w+|(?\'oid\'\\d+))/posts/(?\'pid\'\\w+)!'),true,array(array('extract'=>array('!oid="?(?\'oid\'\\d+)!'),'match'=>array('!//plus\\.google\\.com/(?:u/\\d+/)?\\+[^/]+/posts/\\w!'))),null,true),
		...
		'imgur'=>array('imgur albums and GIFV','http://imgur.com/',array('images'=>1),"!imgur\\.com/a/(?'id'\\w+)!\n!imgur\\.com/(?'id'\\w+)\\.(?:gifv|mp4)!\n!(?'id')imgur\\.com/a/\\w!\n!(?'id')imgur\\.com/\\w+\\.(?:gifv|mp4)!",array('!imgur\\.com/a/(?\'id\'\\w+)!','!imgur\\.com/(?\'id\'\\w+)\\.(?:gifv|mp4)!'),true,array(array('extract'=>array('!<a class="(?\'type\'album)!'),'match'=>array('!imgur\\.com/a/\\w!')),array('extract'=>array('!width:\\s*(?\'width\'\\d+)!','!height:\\s*(?\'height\'\\d+)!','!(?\'type\'gifv)!'),'match'=>array('!imgur\\.com/\\w+\\.(?:gifv|mp4)!'),'url'=>'http://i.imgur.com/{@id}.gifv'))),
		...
		'reddit'=>array('Reddit comment permalink','http://www.reddit.com/',array('social'=>1),"!(?'id')(?=.*?[./]reddit\\.com[:/]).*?(?'path'/r/\\w+/comments/\\w+/\\w+/\\w+)!\n!(?'id')(?=.*?[./]reddit\\.com[:/]).*?/comments/!",array('!(?=.*?[./]reddit\\.com[:/]).*?(?\'path\'/r/\\w+/comments/\\w+/\\w+/\\w+)!'),true,array(array('extract'=>array('!data-token="(?\'token\'\\w+)!'),'match'=>array('!/comments/!')))),
	...
	*/
	...
	{
		$vars += array('path' => null, 'token' => null);

		$html='<iframe width="800" height="165" src="//s9e.github.io/iframe/reddit.min.html#'.htmlspecialchars($vars['path'],2).'#'.htmlspecialchars($vars['token'],2).'" onload="var a=Math.random();window.addEventListener(\'message\',function(b){if(b.data.id==a)style.height=b.data.height+\'px\'});contentWindow.postMessage(\'s9e:\'+a,src.substr(0,src.indexOf(\'/\',8)))" allowfullscreen="" frameborder="0" scrolling="no" data-s9e=""></iframe>';
