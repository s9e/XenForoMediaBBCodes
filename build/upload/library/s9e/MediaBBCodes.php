...
class s9e_MediaBBCodes
{
	...
	*/
	public static $sites = array(
		...
		'imdb'=>array('IMDb trailers','https://www.imdb.com/trailers/',array('movies'=>1),'!imdb\\.com/[/\\w]+?/vi(?P<id>\\d+)!',array(),true,array(array('extract'=>array('!imdb\\.com/[/\\w]+?/vi(?P<id>\\d+)/[/\\w]*?embed!'),'match'=>array('!imdb\\.com/[/\\w]+?/vi(?P<id>\\d+)!'),'url'=>'https://www.imdb.com/video/imdb/vi{@id}/imdb/embed')),'<span data-s9e-mediaembed="imdb" style="display:inline-block;width:100%;max-width:640px"><span style="display:block;overflow:hidden;position:relative;padding-bottom:56.25%"><iframe allowfullscreen="" loading="lazy" scrolling="no" src="//www.imdb.com/video/imdb/vi{$id}/imdb/embed?autoplay=false&amp;width=640" style="border:0;height:100%;left:0;position:absolute;width:100%"></iframe></span></span>'),
		...
		'spotify'=>array('Spotify','https://www.spotify.com/',array('music'=>1,'podcasts'=>1),"!(?:open|play)\\.spotify\\.com/(?:user/[-.\\w]+/)?(?P<id>(?:album|artist|episode|playlist|show|track)(?:[:/][-.\\w]+)+)!\n!(?P<id>)link\\.tospotify\\.com/.!",array('!(?:open|play)\\.spotify\\.com/(?:user/[-.\\w]+/)?(?P<id>(?:album|artist|episode|playlist|show|track)(?:[:/][-.\\w]+)+)!'),true,array(array('extract'=>array('!(?:open|play)\\.spotify\\.com/(?:user/[-.\\w]+/)?(?P<id>(?:album|artist|episode|playlist|show|track)(?:[:/][-.\\w]+)+)!'),'match'=>array('!link\\.tospotify\\.com/.!')))),
		...
		'stitcher'=>array('Stitcher','https://www.stitcher.com/',array('podcasts'=>1),"!(?P<id>)(?=.*?[./]stitcher\\.com[:/]).*?/(?:embed|splayer/f)/(?P<fid>\\d+)/(?P<eid>\\d+)!\n!(?P<id>)(?=.*?[./]stitcher\\.com[:/]).*?/(?:podcast/|show/|s\\?)!",array('!(?=.*?[./]stitcher\\.com[:/]).*?/(?:embed|splayer/f)/(?P<fid>\\d+)/(?P<eid>\\d+)!'),true,array(array('extract'=>array('!/embed/(?P<fid>\\d+)/(?P<eid>\\d+)!'),'match'=>array('!/(?:podcast/|show/|s\\?)!')))),
		...
		'twitter'=>array('Twitter','https://twitter.com/',array('social'=>1),'@(?:twitter|x)\\.com/(?:#!/|i/)?\\w+/(?:status(?:es)?|tweet)/(?P<id>\\d+)@',array('@(?:twitter|x)\\.com/(?:#!/|i/)?\\w+/(?:status(?:es)?|tweet)/(?P<id>\\d+)@')),
	...
	*/
	...
	public static function renderDemocracynow($vars)
	{
		...
		$html='<span data-s9e-mediaembed="democracynow" style="display:inline-block;width:100%;max-width:640px"><span style="display:block;overflow:hidden;position:relative;padding-bottom:56.25%"><iframe allowfullscreen="" loading="lazy" scrolling="no" style="border:0;height:100%;left:0;position:absolute;width:100%" src="//www.democracynow.org/embed/';if(str_contains($vars['id'],'/headlines'))$html.='headlines/'.htmlspecialchars(strstr($vars['id'],'/headlines',true),2);elseif(str_starts_with($vars['id'],'2'))$html.='story/'.htmlspecialchars($vars['id'],2);elseif(str_starts_with($vars['id'],'shows/'))$html.='show/'.htmlspecialchars(substr(strstr($vars['id'],'/'),1),2);else$html.=htmlspecialchars($vars['id'],2);$html.='"></iframe></span></span>';
	...
	public static function renderSpotify($vars)
	{
		...
		$html='<iframe data-s9e-mediaembed="spotify" allow="encrypted-media" allowfullscreen="" loading="lazy" scrolling="no" src="https://open.spotify.com/embed/'.htmlspecialchars(strtr($vars['id'],':','/').$vars['path'],2).'" style="border:0;border-radius:12px;height:';if(str_starts_with($vars['id'],'episode')||str_starts_with($vars['id'],'show')||str_starts_with($vars['id'],'track'))$html.='152';else$html.='380';$html.='px;max-width:900px;width:100%"></iframe>';
	...
	public static function renderStitcher($vars)
	{
		...
		$html='<iframe data-s9e-mediaembed="stitcher" allowfullscreen="" loading="lazy" scrolling="no" src="https://www.stitcher.com/embed/'.htmlspecialchars($vars['fid'],2).'/'.htmlspecialchars($vars['eid'],2).'" style="border:0;height:150px;max-width:900px;width:100%"></iframe>';
	...
	public static function renderTwitter($vars)
	{
		...
		$html='<iframe data-s9e-mediaembed="twitter" allow="autoplay *" allowfullscreen="" loading="lazy" onload="let c=new MessageChannel;c.port1.onmessage=e=&gt;this.style.height=e.data+\'px\';this.contentWindow.postMessage(\'s9e:init\',\'*\',[c.port2])" scrolling="no" style="border:0;height:273px;max-width:550px;width:100%" src="https://s9e.github.io/iframe/2/twitter.min.html#'.htmlspecialchars($vars['id'],2);if(XenForo_Application::get('options')->s9e_MEDIAEMBED_THEME==='dark')$html.='#theme=dark';$html.='"></iframe>';
