...
class s9e_MediaBBCodes
{
	...
	*/
	public static $sites = array(
		...
		'dailymotion'=>array('Dailymotion','http://www.dailymotion.com/',array('misc'=>1),'!dailymotion\\.com/(?:live/|user/[^#]+#video=|video/)(?\'id\'[A-Za-z0-9]+)!',array('!dailymotion\\.com/(?:live/|user/[^#]+#video=|video/)(?\'id\'[A-Za-z0-9]+)!'),7=>'<iframe width="560" height="315" src="//www.dailymotion.com/embed/video/{$id}" allowfullscreen="" frameborder="0" scrolling="no"></iframe>'),
		...
		'gist'=>array('GitHub Gist (via custom iframe)','https://gist.github.com/',array('misc'=>1),'!gist\\.github\\.com/(?\'id\'(?:\\w+/)?[\\da-f]+(?:/[\\da-f]+)?)!',array('!gist\\.github\\.com/(?\'id\'(?:\\w+/)?[\\da-f]+(?:/[\\da-f]+)?)!'),7=>'<iframe style="width:100%" src="//s9e.github.io/iframe/gist.min.html#{$id}" frameborder="0" onload="var a=Math.random();window.addEventListener(\'message\',function(b){if(b.data.id==a)style.height=b.data.height+\'px\'});contentWindow.postMessage(\'s9e:\'+a,src.substr(0,src.indexOf(\'/\',8)))"></iframe>'),
		...
		'npr'=>array('NPR','http://www.npr.org/',array('misc'=>1),'!(?\'id\')npr\\.org/[\\w/]+/\\d+!',array(),true,array(array('extract'=>array('!openPlayer\\((?\'i\'\\d+), (?\'m\'\\d+)!'),'match'=>array('!npr\\.org/[\\w/]+/\\d+!')))),
		...
		'podbean'=>array('Podbean','http://www.podbean.com/',array('podcasts'=>1),"!podbean\\.com/site/player/index/pid/\\d+/eid/(?'id'\\d+)!\n!(?'id')podbean\\.com/e/!",array('!podbean\\.com/site/player/index/pid/\\d+/eid/(?\'id\'\\d+)!'),true,array(array('extract'=>array('!embed/postId/(?\'id\'\\d+)!'),'match'=>array('!podbean\\.com/e/!'))),'<iframe width="100%" height="100" style="max-width:900px" src="//www.podbean.com/media/player/audio/postId/{$id}" allowfullscreen="" frameborder="0" scrolling="no"></iframe>'),
		...
		'zippyshare'=>array('Zippyshare audio files','http://www.zippyshare.com/',array('misc'=>1),'!(?\'id\')(?=.*?[./]zippyshare\\.com[:/]).*?/v/!',array(),true,array(array('extract'=>array('!file=(?\'file\'\\w+)&amp;server=(?\'server\'\\d+)!'),'match'=>array('!/v/!'))))
	);
